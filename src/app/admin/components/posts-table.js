...
import { deletePostApi } from "@/actions/post-api";
...
export default function PostsTable({ postsList, setPostsList }) {
  ...
  const handleDeleteClick = (row) => {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        setPostsList(
          postsList.filter((postsList) => postsList._id !== row._id)
        );
      }
    });
  };
  ...
  return <DataTable columns={columns} data={postsList} pagination responsive />;
}
